    ...
    case msg of
        ...
        RootNode ->
            let
                childNodes =
                    model.nodes
                        |> Dict.foldl (\k n c -> c ++ n.childNodes) []

                keys =
                    model.nodes
                        |> Dict.foldl (\k n c -> if List.length n.childNodes > 0 then c ++ [k] else c) []
            in
            ( { model
                | rootNode =
                    keys
                        |> List.foldl
                            (\k c ->
                                if c == Nothing && not (List.member k childNodes) then Just k else c
                            )
                            Nothing
                }
            , Cmd.none
    ...
    input
        ...
        |> List.foldl
            (\inputLine dict ->
                let
                    matched =
                        inputLine |> find All (regex "^([a-z]+)\\s+\\(([0-9]+)\\)(\\s+\\-\\>\\s+([a-z, ]*))?$")
                in
                case List.head matched of
                    Just match ->
                        case match.submatches of
                            (Just key)::(Just weight)::_::keys::[] ->
                                let
                                    childNodes =
                                        case keys of
                                            Just k ->
                                                k |> String.split "," |> List.map String.trim
