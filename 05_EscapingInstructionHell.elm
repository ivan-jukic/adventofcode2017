...
findNextInstruction current instructions =
    instructions
        |> List.indexedMap (,)
        |> List.foldl
            (\(idx, i) c ->
...
updateInstruction : Int -> Int -> List Int -> List Int
updateInstruction idx newValue instructions =
    instructions
        |> List.indexedMap (,)
        |> List.map
            (\(i, oldValue) ->
...
getInput : List Int
getInput =
    input |> String.split " " |> List.map (\n -> n |> String.toInt |> Result.withDefault 0)
