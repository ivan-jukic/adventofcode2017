...
calculate checkAnagrams =
    let
        repetitions word list =
            list |> List.foldl
                (\w c -> if word == w || isAnagram word w then c + 1 else c ) 0

        isAnagram w1 w2 =
            checkAnagrams
                && w1 /= w2
                && String.length w1 == String.length w2
                && (dictFromWord w1 == dictFromWord w2)
        ...
        |> List.foldl
            (\passphrase total ->
                let
                    ...
                    isValid =
                        words |> List.foldl (\w c -> if c then not (repetitions w words > 1) else c ) True
                in
                if isValid then total + 1 else total
            )
            0
